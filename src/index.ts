...
import os from 'os';
import { createEmailMessage, createEmailWithNodemailer } from "./emailUtils.js";
import { createLabel, updateLabel, deleteLabel, listLabels, findLabelByName, getOrCreateLabel, GmailLabel } from "./labelManager.js";
import { createFilter, listFilters, getFilter, deleteFilter, filterTemplates, GmailFilterCriteria, GmailFilterAction } from "./filterManager.js";
import {
    getFeedPosts as linkedinGetFeedPosts,
    searchJobs as linkedinSearchJobs
...
const GetFeedPostsSchema = z.object({
    limit: z.number().optional().default(10).describe("Maximum number of feed posts to retrieve"),
    offset: z.number().optional().default(0).describe("Number of posts to skip"),
}).describe("Retrieves LinkedIn feed posts");

const SearchJobsSchema = z.object({
    keywords: z.string().describe("Job search keywords"),
    limit: z.number().optional().default(3).describe("Maximum number of job results"),
    offset: z.number().optional().default(0).describe("Number of results to skip"),
    location: z.string().optional().default('').describe("Optional location filter"),
}).describe("Searches for jobs on LinkedIn");
...
async function main() {
    ...
    // ─── Tool Registration (all camelCase) ───────────────────────────────────
    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            ...
            {
                name: "getFeedPosts",
                description: "Retrieves LinkedIn feed posts",
                inputSchema: zodToJsonSchema(GetFeedPostsSchema),
            },
            {
                name: "searchJobs",
                description: "Searches for jobs on LinkedIn",
                inputSchema: zodToJsonSchema(SearchJobsSchema),
            },
        ],
    }));
        ...
        try {
            switch (name) {
                ...
                case "draftEmail": {
                    ...
                    const action = name === "sendEmail" ? "send" : "draft";
                    ...
                    const processAttachmentParts = (part: GmailMessagePart, partPath: string = '') => {
                        ...
                        if (part.parts) {
                            part.parts.forEach((subpart: GmailMessagePart) =>
                                processAttachmentParts(subpart, `${partPath}/parts`)
                            );
                        }
                    };
                ...
                case "getFeedPosts": {
                    const validatedArgs = GetFeedPostsSchema.parse(args);
                    const result = await linkedinGetFeedPosts(validatedArgs.limit, validatedArgs.offset);
                ...
                case "searchJobs": {
                    ...
                    const result = await linkedinSearchJobs(
                        validatedArgs.keywords,
                        validatedArgs.limit,
